...
var LoginView = cc.Class({
    ...
    onLoad() {
        ...
        //     if (require("Util").getAPI_AVAILABLE()) {
        //         this.btn_login_main.node.position = cc.v2(200, 60);
        //         this.btn_playAppleId.node.active = true;
        //     }
        // }

        cc.loader.loadResDir("logo/anim_iconJS", (count, totalcount) => { }, (err, prefabs) => {
            if (err) return;
            cc.NGWlog("load xong dir logo");
        });
    },
    onTest() {
    ...
    onEnable() {
        ...
        //require("GameManager").getInstance().is_ltv = false;
        // require('UIManager').instance.onHideLoadLtv();
        //clearTimeout(require("GameManager").getInstance().login_timeout);
        cc.NGWlog("Enable loginView");
        ...
        if (languaSave !== null) {
            languaSave = parseInt(languaSave);
            indexLanguae = languaSave;
        }
        ...
        // require('GameManager').getInstance().setCurView(CURRENT_VIEW.LOGIN_VIEW);
        require('TrackingManager').SendTracking(TRACKING_TYPE.AppLauncher);
        ...
        // if (GameManager.isReconnect) {

        //     switch (GameManager.typeLogin) {
        //         case LOGIN_TYPE.FACEBOOK: {
        //             this.onClickLoginFacebook();
        //             break;
        //         }
        ...
        //     }
        // }
        // GameManager.isReconnect = false;
        let hotLine=cc.sys.localStorage.getItem("hotline");
        if(hotLine!=null && typeof hotLine!="undefined"){
            this.lb_Support.node.active=true;
            this.lb_Support.string=hotLine;
        }
    },
    setIsPrivate() {
    ...
    onClickLoginAppleID() {
        ...
        GameManager.typeLogin = LOGIN_TYPE.APPLE_ID;
        cc.sys.localStorage.setItem("lastLogin", LOGIN_TYPE.APPLE_ID);
        if (cc.sys.isNative) {
            this.isClickLogin = true;
            require('Util').signWithApple();
        }
    },

...
cc.Class({
    ...
    init(data) {
        ...
        this.listData = data;
        let str = 'TOP GAMER ' + this.listData.name;
        if (this.listData.name) this.lbTitle.string = str.toUpperCase();
        this.updateList();
        ...
        var listTopGamerData = GameManager.getInstance().list_topgamer;
        ...
        for (let i = 0; i < listTopGamerData.length; i++) {
            if (listTopGamerData[i].gameid === this.listData[0].Gameid) {
                GameManager.getInstance().loadTextureFromUrl(this.spReward, listTopGamerData[i].url_img_js);
    ...
    updateList() {
        ...
        for (let index = 0; index < this.listData.length; index++) {
            ...
            if (this.listData[index].N === GameManager.getInstance().user.uname && !this.setPlayer) {
                this.myItem = itemRich;
                this.setUserData(this.listData[index]);
            } else if (this.listData[index].N === GameManager.getInstance().user.uname && this.setPlayer) {
    ...
    setUserData(playerData) {
        ...
        this.userAvatar.node.getComponent("AvatarItem").loadTexture(avtId, playerData.N, playerData.Faid);

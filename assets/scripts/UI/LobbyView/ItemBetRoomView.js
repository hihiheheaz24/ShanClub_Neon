...
cc.Class({
    ...
    setInfo: function (mark, _require, cur_user, is_select, is_best, index, maxAgCon, ag) {
        ...
        if (gameCur === GAME_ID.BACCARAT || gameCur === GAME_ID.BLACKJACK) {
            this.lb_require.string = GameManager.getInstance().formatMoney(_require) + "+";
        }
    ...
    isnomal() {
        ...
        this.bg_Select.node.setContentSize(cc.size(231,130));
        this.lb_require.node.active = true;
        this.lb_cur_user.node.setPosition(-2, -48);
        this.ic_user.node.setPosition(-21, -47);
        this.lb_cur_user.node.color = new cc.Color(255, 255, 255);
        this.ic_user.node.color = new cc.Color(255, 255, 255);
        this.lb_mark.node.setPosition(-2, -6);
    },
    isVip(indexSpr) {
        var num = 0;
        // if (this.index % 2 === 0)
        ...
        this.bg_Select.node.setContentSize(cc.size(231,110));
        this.lb_require.node.active = false;

        this.lb_cur_user.node.setPosition(-2, -30);
        this.ic_user.node.setPosition(-21, -27);
        this.lb_cur_user.node.color = new cc.Color(25, 11, 124);
        this.ic_user.node.color = new cc.Color(25, 11, 124);
        this.lb_mark.node.setPosition(-2, 9);
        this.sp_best_choise.node.active = false;
        this.ic_best.node.active = false;
    },
    onClickChoose() {
        ...
        if (this.isSelect === false) {
            cc.NGWlog("user.nmAg=" + GameManager.getInstance().user.nmAg);
            cc.NGWlog("user.countMailAg==" + Global.FreeChipView.countMailAg);

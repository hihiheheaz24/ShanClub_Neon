...
var PlayerViewCasino = cc.Class({
    ...
    effLbName() {
        ...
        cc.NGWlog("WIDTH LB NAME=="+this.lbName.node.width);
    ...
    onClickAvatar() {
        ...
        if (GameManager.getInstance().gameView.stateGame === STATE_GAME.VIEWING) return;
    ...
    setName(strName) {
        ...
        cc.NGWlog("Chay vao ham set name");
        this.effLbName();
    ...
    setAvatar(avaId, name, Faid) {
        this.imgAvatar.node.getComponent("AvatarItem").loadTexture(avaId, name, Faid);
    },
    ...
    showEffectWinLose(isWin) {
        cc.NGWlog("Chay vao set EffectWinLose");
        ...
        actionNode.runAction(cc.sequence(cc.delayTime(2.5), cc.removeSelf()));
    },
    ...
    effectFlyMoney(money, fontSize = 50, moveTo = 50, posX = 0, posY = 0) { //Show fly money effect
        ...
        labelText.font = font;
        nodeText.position = this.node.position.add(cc.v2(posX, posY));
        this.node.getParent().addChild(nodeText, GAME_ZORDER.Z_TEXT_FLY);
    ...
    setScore(score) {
        // cc.NGWlog('==========> socre:   ' + score);
        if (score < 0) {
            this.lbScore.node.getParent().active = false;
            return;
        }
        this.lbScore.node.getParent().active = true;
        this.lbScore.string = score + '';
    },

    setDeclare(score) {
        this.setScore(score);
        this.lbDeclare.node.setScale(2.0);
        this.lbDeclare.string = cc.js.formatStr('%s %d', require('GameManager').getInstance().getTextConfig('txt_lose'), score);
        this.lbDeclare.node.runAction(cc.sequence(cc.delayTime(.2), cc.scaleTo(.1, 1.0)));
    }
});

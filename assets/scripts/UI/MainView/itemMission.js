cc.Class({
    ...
    onClickMission() {
        ...
        if (this.missionType !== 0) {
            if (this.clickCount === 0) {
                this.clickCount++;
                this.node.setContentSize(cc.size((this.node.width + this.bkgInfo.width ) -151, this.node.height));
        ...
        switch (this.missionType) {
            ...
            case 1:
                ...
                this.lb_yourCodeInfo.node.getParent().position = cc.v2(-40, -10);
                this.btn1.node.active = require("GameManager").getInstance().is_bl_fb;
                this.btn2.node.active = true;
                this.lbInfo.node.position = cc.v2(0, 152);
                ...
                this.btn1.node.position = cc.v2(-30, this.bkgInfo.y - 80);
                ...
                textReward = cc.js.formatStr(require('GameManager').getInstance().getTextConfig('txt_node_share_code'), require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agInvite), require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agInviteFr));
                this.lbReward.string = textReward;
                break;
            case 2:
                Global.MissionView.scollView.scrollToRight(0.1);
                this.imgInfo.active = true;
                this.imgInfo.getChildByName("bg_check").getComponent(cc.Sprite).spriteFrame = this.listImgInfo[1];
                this.btn1.node.setContentSize(cc.size(260, 100));
                this.btn1.node.active = true;
                this.btn1.node.position = cc.v2(this.imgInfo.x, this.imgInfo.y - 145);
                this.btn1.node.getChildByName('lbBtn1').getComponent(cc.Label).string = require('GameManager').getInstance().getTextConfig('txt_contact_admin');
                this.btn1.node.getChildByName('lbBtn1').position = cc.v2(0, 0);
                this.btn1.getComponent(cc.Sprite).spriteFrame = this.listImgInfo[3];
                this.lbInfo.node.setContentSize(cc.size(440, 40));
                this.lbInfo.node.position = cc.v2(-20, this.btn1.node.y - 90);
                this.lbInfo.fontSize = 18;
                this.lbInfo.string = require('GameManager').getInstance().getTextConfig('txt_node_contact_admin')
                break;
            case 3:
                Global.MissionView.scollView.scrollToRight(0.1);
                this.imgInfo.active = true;
                ...
                this.btn1.node.active = true;
                this.btn1.node.position = cc.v2(this.imgInfo.x, this.imgInfo.y - 145);
                this.btn1.node.setContentSize(cc.size(250, 100));
                this.lbInfo.node.setContentSize(cc.size(440, 40));
                this.lbInfo.node.position = cc.v2(-20, this.btn1.node.y - 90);
                this.lbInfo.fontSize = 18;
                ...
                this.btn1.node.getChildByName('lbBtn1').position = cc.v2(0, 0);
                this.btn1.getComponent(cc.Sprite).spriteFrame = this.listImgInfo[2];
                //this.lbReward.string = require('GameManager').getInstance().getTextConfig('txt_node_share_photo');
                this.lbInfo.string = cc.js.formatStr(require('GameManager').getInstance().getTextConfig('txt_key_share_photo'), require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agShareImg));

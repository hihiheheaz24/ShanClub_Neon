...
cc.Class({
  ...
  onLoad(){
    ...
    this.handleLuckyMove();
    this.isLuckyMove = false;
  },
  
  handleLuckyMove (){
    let orginPos = null;
    this.btn_lucky_number.on(cc.Node.EventType.TOUCH_START, (touch) => {
      orginPos = this.btn_lucky_number.position;
    }, this);

    this.btn_lucky_number.on(cc.Node.EventType.TOUCH_MOVE, (touch) => { 
      cc.log('chay vao touch moveeee');
      let delta = touch.getDelta();
      this.btn_lucky_number.x += delta.x;
      this.btn_lucky_number.y += delta.y;
      if(Math.sqrt(Math.pow(this.btn_lucky_number.x - orginPos.x,2)) >= 10
      || Math.sqrt(Math.pow(this.btn_lucky_number.y- orginPos.y,2)) >= 10){
        this.isLuckyMove = true;
      }else{
        this.isLuckyMove = false;
      }
    }, this);
    this.btn_lucky_number.on(cc.Node.EventType.TOUCH_END, (touch) => {
      orginPos = null;
    }, this);
  },
  ...
  getInfoGame() {
    ...
    require('NetworkManager').getInstance().getMail(10);
  },
  onClickTest() {
  ...
  startGame() {
    // require('UIManager').instance.process_load_ltv.node.active = true;
    // require('UIManager').instance.process_load_ltv.onShow();
    cc.NGWlog('chay vao gham StartGame');
    this.loadGameList();
    setTimeout(() => {
      this.setInfoButonWithConfig();
    }, 1000);
    if(require("GameManager").getInstance().is_dt 
    && require("GameManager").getInstance().user.vip >= GameManager.getInstance().v_dt){
    ...
    let vSauBanh = require("GameManager").getInstance().vsaubanh;
    Global.KetView.initWithConfig();
    // this.btn_sendGift.node.active = myVip >= vSauBanh ? true : false;
   this.btn_safe.node.active = myVip >= vSauBanh ? true : false;
    this.btn_lucky_number.active = require("GameManager").getInstance().is_xs;

    this.btn_giftCode.node.active = require("GameManager").getInstance().ismaqt;

    this.redDotChatWorld.active = false;
    this.redDotMail.active = false;
    this.redDotFriend.active = false;
    ...
    this.scheduleOnce(() => {
      ...
      // if (require("GameManager").getInstance().user.vip >= 1 && require('GameManager').getInstance().typeLogin === LOGIN_TYPE.PLAYNOW) require("GameManager").getInstance().onShowProfile(true);
      // if (
      //   (require("GameManager").getInstance().user.vip >= require("GameManager").getInstance().vip_rename ||
      //     (require("GameManager").getInstance().user.vip == 1
      //       && require("GameManager").getInstance().user.ag > require("GameManager").getInstance().ag_rename)
      //     || (require("GameManager").getInstance().user.vip == 1
      //       && require("GameManager").getInstance().user.ag < require("GameManager").getInstance().ag_rename_min))
      //   && (require("GameManager").getInstance().user.uname.indexOf("fb.") != -1 || GameManager.getInstance().user.uname.indexOf("te.") != -1)) {
      ...
      if ((condi0 || condi1 || condi2 || condi3) && condi4) {
        cc.NGWlog("Mo register len cho tao.user name cua tao la:" + GameManager.getInstance().user.uname);
        UIManager.instance.onshowRegister();
      }
    ...
    // if (UIManager.instance.arrayDataBanner.length > 0) {
    //   UIManager.instance.indexCurrentDataBanner = 0;
    //   UIManager.instance.showBanner();
    // }

  },
  setInfoButonWithConfig() {
    this.loadGameList();
    if(require("GameManager").getInstance().is_dt 
    && require("GameManager").getInstance().user.vip >= GameManager.getInstance().v_dt){
      this.btn_cashOut.node.active = true;
  ...
  setInfo() {
    ...
    this.scheduleOnce(() => {
      if (GameManager.getInstance().user.nmAg > 0 || Global.FreeChipView.countMailAg > 0) {
        this.lbFreeChip.node.color = new cc.Color(255, 255, 0);
      ...
      this.lbFreeChip.node.color = new cc.Color(255, 255, 255);
      //cc.NGWlog('=============> count mail ag la' + Global.FreeChipView.countMailAg + '   nmag la ===> ' + GameManager.getInstance().user.nmAg)  
  ...
  updateName() {
    ...
    cc.NGWlog("Ten displayName la: " + this.lbName.string);
  ...
  loadGameList() {
    cc.NGWlog('chay vao ham load game=========================================================================');
  ...
  setAvatar() {
    ...
    this.avatar.node.getComponent("AvatarItem").loadTexture(avtId, myName);
  },
  ...
  setTimeGetMoney() {
    try {
      if (require("GameManager").getInstance().promotionInfo.time <= 0) {
        ...
        this.animChipOnline.setAnimation(0, "stay", true);
        ...
        if (require("GameManager").getInstance().promotionInfo.time > 0) {
          this.animChipOnline.setAnimation(0, "run", true);
  ...
  onClickCallAdmin() {
    require('SoundManager1').instance.playButton();
    if(GameManager.getInstance().is_bl_fb){
  ...
  onShowLotoView() {
    if(this.isLuckyMove == true){
      this.isLuckyMove = false;
      return;
    }
    UIManager.instance.onShowLotoView();
    require('SoundManager1').instance.playButton();
  },
  onClickTopRich() {
  ...
  onClickExchange() {
    require('SMLSocketIO').getInstance().emitSIOCCC(cc.js.formatStr("ClickShowExchange_%s", require('GameManager').getInstance().getCurrentSceneName()));
    require('SoundManager1').instance.playButton();
    UIManager.instance.onShowExchange();
  },
  onClickChatWorld() {

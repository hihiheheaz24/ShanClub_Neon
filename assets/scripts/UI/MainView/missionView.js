cc.Class({
    ...
    setInfo() {
        this.initData();
        this.is_login_fb = require('GameManager').getInstance().is_login_fb;
    ...
    initData() {
        ...
        this.avt.node.getComponent("AvatarItem").loadTexture(idAva, name);

        let listSize = [];

        if (require("GameManager").getInstance().is_bl_fb) {
            listSize.push('shareImage');
            listSize.push('contactAdmin');
        }
        ...
        if (listSize.length === 3) {
            ...
            this.scollView.content.getComponent(cc.Layout).spacingX = 0;
        ...
        for (let i = 0; i < listSize.length; i++) {
            ...
            switch (i) {
                case 3:
                    ...
                    text = require('GameManager').getInstance().getTextConfig('txt_info_verify_acc');
                    chip = require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agVerify) + '';
                    break;
                case 2:
                    ...
                    chip = require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agInvite) + '';
                    break;
                case 1:
                    ...
                    chip = require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agContactAd) + '';
                    break;
                case 0:
                    ...
                    chip = require('GameManager').getInstance().formatMoney(require("GameManager").getInstance().agShareImg) + '';
                    break;
            }
    ...
    shareCodeMessage(yourCode) {
        require('SoundManager1').instance.playButton();
        require('Util').shareCodeMessage(yourCode + "\n" + require('GameManager').getInstance().newest_versionUrl);
    },

...
var RECT_SIZE = cc.v2(175, 139);
var SPEED_NORMAL = 0.15;
var SPEED_AUTO = 0.1;


var Slot20NoelView = cc.Class({
    ...
    onLoad() {
        ...
        this.listSpriteId = [];
        this.listPosItem = [cc.v2(115, 237), cc.v2(320, 237), cc.v2(526, 237), cc.v2(732, 237), cc.v2(934, 237)];
        this.listAnimWS = [];
    ...
    update(dt) {
        if (this.isHoldSpin) {
            ...
            if (this.timeHoldSpin < 1 && this.timeHoldSpin > 0) {
                this.onClickSpin();
    ...
    initItemSlot(dataView) {
        let delta = 0;
        this.listCollum = [];
        cc.NGWlog("SLOT NOEL:listItem size=" + this.listItem.length);
        for (let i = 0; i < this.listItem.length; i++) {
            // this.listItem[i].node.destroy();
        }
        this.listItem = [];
        for (let i = 0; i < 15; i++) {
            let item1 = cc.instantiate(this.itemSlotPr).getComponent("ItemSlot20Noel");
            let item2 = cc.instantiate(this.itemSlotPr).getComponent("ItemSlot20Noel");
            let item3 = cc.instantiate(this.itemSlotPr).getComponent("ItemSlot20Noel");
        ...
        let acSpin = cc.callFunc(() => {
            ...
            for (let i = 0; i < this.listCollum.length; i++) {
                ...
                for (let j = 0; j < collum.length; j++) {
                    let item = collum[j].getComponent("ItemSlot20Noel");
                    ...
                    let acCheck = cc.callFunc(() => {
                        ...
                        if (!this.listCheckStopCol[i] && item.numCol === i) { //running
                            ...
                            if (i === 2 && this.isCheckScatterItem3) {
                                ...
                                timedel = 2;
                                cc.loader.loadRes(ResDefine.slot_speedspin_8818, sp.SkeletonData, (err, aniClip) => {
                                    if (err) {
                                        return;
                                    }
                                    let skeData = aniClip;
                                    let data = {
                                        dir: ResDefine.slot_speedspin_8818,
                                        skeData: skeData
                                    }
                                    this.skeData.push(data);
                                    this.setInfoAnim(this.firstAnim, skeData, cc.v2(this.posAnimSpeedX - 5, 50), cc.v2(1, 0.98));
    ...
    setStateSpinBtn(forceState = null) {
        ...
        switch (state) {
            ...
            case "autospin":
                this.animBtnSpin.node.active = false;
                this.animBtnSpin.playAnimation2("autospin", true);
    ...
    handleSpin(data) {
        // data = {
        //     evt: "slotViews",
        //     slotViews: [[9,7,1],[2,4,11],[4,6,0],[3,7,3],[6,8,2]],
        //     creditWin: 25,
        //     winningLine: [5],
        //     lineDetail: [{ lineId: 13, win: 25 },{ lineId: 19, win: 25 }],
        //     freeSpinLeft: 0,
        //     winType: 0,
        //     freeSpin: false,
        //     agWin: 25,
        ...
        //         agUser: 1461
        //     },
        //     MarkBet: [1, 5, 50, 500, 5000, 50000, 500000]
        // }
        //    this.spinCountCheck++;
    ...
    showAnimScatter() {
        ...
        for (let i = 0; i < this.listScatter.length; i++) {
            ...
            cc.loader.loadRes("game/8818/SlotNoel/12/skeleton", sp.SkeletonData, (err, aniClip) => {
                if (err) {
                    return;
                }
                let skeData = aniClip;
            ...
            this.node.runAction(cc.sequence(cc.delayTime(i * 1.5),
                cc.callFunc(() => {
                    ...
                    for (let i = 0; i < this.listIndexSpr.length; i++) {
                        this.listOriginItem[i].getComponent("ItemSlot20Noel").setStateSpr(this.listIndexSpr[i], true);
                    }
                    this.listIndexSpr=[];
                    ...
                    for (let j = 0; j < lineWin.length; j++) { //[1,2,1,2,1,];
                        let item = this.listOriginItem[j].getComponent("ItemSlot20Noel");
    ...
    showAnim5OfAkind() {
        ...
        this.firstAnim.animation.skeletonData = null;
        cc.loader.loadRes(ResDefine.slot_fiveOfaKind_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_fiveOfaKind_8818,
                skeData: skeData
            }
            this.skeData.push(data);
    ...
    showAnimFreeSpin() {
        ...
        this.firstAnim.node.active = false;
        this.firstAnim.animation.skeletonData = null;
        cc.loader.loadRes(ResDefine.slot_freespinAnim_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_freespinAnim_8818,
                skeData: skeData
            }
            this.skeData.push(data);
            let animName = "animation";
        ...
        for (let i = 0; i < listLineCheckWild.length; i++) {
            ...
            for (let j = 0; j <= countWildCheck; j++) {
                ...
                if (data.lineWinId[j] === 11) {
                    ...
                    let anim = this.getAnimFromPool(this.listAnimWS);
                    cc.loader.loadRes("game/8818/SlotNoel/11/skeleton", sp.SkeletonData, (err, aniClip) => {
                        if (err) {
                            return;
                        }
                        let skeData = aniClip;
    ...
    showAnimBigWin() {
        ...
        this.firstAnim.node.active = false;
        this.firstAnim.animation.skeletonData = null;
        cc.loader.loadRes(ResDefine.slot_MegaBigWin_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_MegaBigWin_8818,
                skeData: skeData
            }
            this.skeData.push(data);
            let animName = "bigwin";
    ...
    showAnimBkgFreeSpin() {
        ...
        cc.loader.loadRes(ResDefine.slot_khungFree_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_khungFree_8818,
                skeData: skeData
            }
            this.skeData.push(data);
        ...
        cc.loader.loadRes(ResDefine.slot_vienFree_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_vienFree_8818,
                skeData: skeData
            }
            this.skeData.push(data);
    ...
    showAnimFreeNum() {
        ...
        cc.loader.loadRes(ResDefine.slot_freenum_8818, sp.SkeletonData, (err, aniClip) => {
            if (err) {
                return;
            }
            let skeData = aniClip;
            let data = {
                dir: ResDefine.slot_freenum_8818,
                skeData: skeData
            }
            this.skeData.push(data);
            ...
            this.lbFreeAnim.string = this.freeSpinLeft + " Free Spin Left";
            this.lbFreeAnim.fontSize = 65;
            this.lbFreeAnim.lineHeight = 90;
            this.node.addChild(animFreeNum.node, 100);
    ...
    initLineWin(lineWin, color) {
        ...
        for (let i = 0; i < lineWin.length; i++) {
            let item = this.listOriginItem[i].getComponent("ItemSlot20Noel");
    ...
    showListAnim(listPos, listId, color, agWin, lineWin) {
        ...
        for (let i = 0; i <= count; i++) {
            ...
            if (i + 1 <= count) {
                if (listPos[i].y > listPos[i + 1].y + 300 || listPos[i].y < listPos[i + 1].y - 300) {
    ...
    turnOffItemSpr(collum, index) {
        ...
        this.listOriginItem[collum].getComponent("ItemSlot20Noel").setStateSpr(index, false);
    },
    ...
    showItemAnim(index, pos, id, agWin, lineWin) {
        ...
        let resPath = "";
        if (id === 11) scale = 0.8;
        resPath = "game/8818/SlotNoel/" + id + "/skeleton";
        animName = "animation";
        let skeData;
    ...
    onClickTest() {
        let vecA = cc.v2(0, 0);
        let vecB = cc.v2(100, 0);
        let vecC = cc.v2(100, 100);
        let vecD = cc.v2(50, -100);
        let vecE = cc.pIntersectPoint(vecA, vecB, vecC, vecD);
        cc.NGWlog("giao diem =" + vecE);

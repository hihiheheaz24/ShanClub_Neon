...
cc.Class({
    ...
    checkListChip(moneyBox) {
        ...
        if(TienConLai === 0 || TienConLai < 0) {
           // require('UIManager').instance.showToast(require('GameManager').getInstance().getTextConfig('txt_all_in'));
            this.listButton[0].node.getComponent(cc.Button).interactable = false;
            this.target.isOkBet = false;
        }
        else{
            this.listButton[0].node.getComponent(cc.Button).interactable = true;
            this.target.isOkBet = true;
        } 

        if(TienConLai < this.listChipBet[0]   && TienConLai > 0) {
            this.target._curChipBet = TienConLai;
            this.listChipBet[0] = TienConLai;
        }
        else{
            this.listChipBet[0] = this._isMinBet;
        }

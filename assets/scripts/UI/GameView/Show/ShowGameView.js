...
var ShowGameView = cc.Class({
    ...
    onLoad() {
        ...
        this.listPosBoxBet = [{ x: 10, y: -75 }, { x: -372, y: -126 }, { x: -359, y: 93 }, { x: 306, y: 72 }, { x: 395, y: -144 }];
        this.listBoxBet = [{}, {}, {}, {}, {}];
    ...
    dealCardPlayer(index, delay , isAction = true, cardCode = 0) {
        ...
        if(!isLeftTable){
            this.node.addChild(cardTemp.node, 20);
        }else{
            this.node.addChild(cardTemp.node);
        }
        
        if (isLeftTable) {
            vTarget = cc.v2(vTemp.x + ((cardTemp.node.getContentSize().width / 2 * 0.38) * player.vectorCardP1.length), vTemp.y);
    ...
    handleFinish(data) {
        ...
        if (!this.thisPlayer.isFold && this.thisPlayer.is_ready) {
                        ...
                        if (player === this.thisPlayer) {
                            ...
                            if (dataListPlayer[t].chipWin > 0 && !this.isCheckAddWinListCard) {
                                ...
                                if (typeof(arrC) != 'undefined') {
                                    ...
                                    let arrCardWin = [];
                                    for (let i = 0; i < this.thisPlayer.vectorCardP1.length; i++) {
                                        arrCardWin.push(this.thisPlayer.vectorCardP1[i].code);
                                    }
                                    this.quickChat.addChatWithCard(str, arrCardWin);
    ...
    HandlerTip(data) {
        ...
        for (let i = 0; i < this.players.length; i++) {
            if (this.players[i].pname == data.N) {
                ...
                this.node.runAction(cc.sequence(cc.delayTime(2), cc.callFunc(() => { this.DealerInGame.show(nameTip, data.AGTip); })))
            }
        }
    },
    onclickSwapCard() {

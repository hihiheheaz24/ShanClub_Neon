...
var BinhGamView = cc.Class({
    ...
    showJackpotWin(name, chip) {
        ...
        player.ag += player._playerView.chipJackpot;
    ...
    ViewIng(dataGame) {
        let strResult = dataGame;
        let dataa = strResult.data;
        this.finishData = JSON.parse(dataa);

        for (let i = 0; i < this.finishData.length; i++) {

            let jpl = this.finishData[i];
            let name = jpl.N;
            let player = this.getPlayer(name);
            ...
            player.ag = jpl.AG - player.mauBinh_M;
            ...
            player.arrBinhWin = [];
            if (jpl.ArrWin.length == 0) continue;

            for (let k = 0; k < jpl.ArrWin.length; k++) {
                let playerData = {};
                playerData.name = jpl.ArrWin[k].Name;
                playerData.score = jpl.ArrWin[k].Score;
                player.arrBinhWin.push(playerData);
            }
        }

        for (let i = 0; i < this.players.length; i++) {
            let player = this.players[i];
            let totalPointBanDau = player.scoreChi1 + player.scoreChi2 + player.scoreChi3 + player.bonusChi1 + player.bonusChi2 + player.bonusChi3;
            player.totalPoint += totalPointBanDau;
            
            if (player === this.thisPlayer && this.stateGame === STATE_GAME.VIEWING) continue;

            for (let j = 0; j < this.players.length; j++) {
                let player1 = this.players[j];
                if (player == player1) continue;
                let point = 0;

                for (let k = 0; k < player.arrBinhWin.length; k++) {
                    let player2 = this.getPlayer(player.arrBinhWin[k].name);
                    if (player1 == player2) {
                        point = player.arrBinhWin[k].score;
                        break;
                    }
                }

                player.totalPoint = player.totalPoint + point;
                player1.totalPoint = player1.totalPoint - point;
            }

            for (let i = 0; i < this.players.length; i++) {
                let player = this.players[i];
                this.setPointTotal(player, this.getDynamicIndex(this.getIndexOf(player)));
            }
        }
        this.initPlayerCard();
    ...
    initPlayerCard() {
        for (let i = 0; i < this.players.length; i++) {
            ...
            for (let j = 0; j < player.vectorCard.length; j++) {
                ...
                if (player != this.thisPlayer && !this.isFinish) {
    ...
    showAnimationSpecialFull() {
        ...
        this.avtSpecial.node.getComponent("AvatarItem").loadTexture(this.thisPlayer.avatar_id, this.thisPlayer.displayName, this.thisPlayer.fid);
        ...
        this.lbNameWin.string = this.thisPlayer.displayName.substring(0,20) + "...";
    ...
    setPointTotal(player, index) {
        ...
        if (this.stateGame == STATE_GAME.VIEWING && player == this.thisPlayer) return;

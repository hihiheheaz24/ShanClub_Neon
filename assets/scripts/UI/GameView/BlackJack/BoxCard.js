    ...
    InstantiateChipWithMoney(_money, node, chipWin = false) {
        ...
        for (i; i >= 0; i--) {
            ...
            if (money >= chip) {
                ...
                let spr = null;
                for (let k = require("GameManager").getInstance().gameView.listCurChip.length - 1; k >= 0; k--) {
                    if (chip == require("GameManager").getInstance().gameView.listCurChip[k]) {
                ...
                this.createLabelChip(item, chip);
                cpSp.spriteFrame = spr;

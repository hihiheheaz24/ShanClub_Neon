...
cc.Class({
    ...
    onClickBack() {
        ...
        if (this.game_state !== STATE_GAME.PLAYING || require('GameManager').getInstance().curGameId === GAME_ID.BLACKJACK) {
            if (require('GameManager').getInstance().curGameId === GAME_ID.SHANKOEMEE
                && require('GameManager').getInstance().gameView.isBanker == true
                && require('GameManager').getInstance().gameView.potValue > 0) {
                GameManager.getInstance().onShowWarningDialog(
                    GameManager.getInstance().getTextConfig('shan2_warning_leave'),
                    DIALOG_TYPE.TWO_BTN,
                    GameManager.getInstance().getTextConfig('txt_yes'),
                    () => {
                        cc.NGWlog("chay vao send auto exit game ========================");
                        require('NetworkManager').getInstance().sendExitGame();
                    },
                    GameManager.getInstance().getTextConfig('txt_no')
                );
            } else {
                cc.NGWlog("chay vao send auto exit game ========================");
                require('NetworkManager').getInstance().sendExitGame();
    ...
    doExitLater() {
        cc.NGWlog("chay vao send auto exit game ========================2");
        let str = '';

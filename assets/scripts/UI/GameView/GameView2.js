...
var GameView2 = cc.Class({
            ...
            if (err) {
                ...
                for (let i = 1; i < this.listPosView.length; i++) {
                    ...
                    if (require("GameManager").getInstance().curGameId == GAME_ID.SHAN_PLUS) {
                        if (i == this.listPosView.length - 1)
    ...
    handleChatTable(data) {
        ...
        if (datName.length > 20) datName = datName.substring(0, 20) + '...';

        if (datMSG !== null && datMSG !== '' && typeof datMSG !== 'undefined') {
            var str = datName + ": " + datMSG;
    ...
    onLeave() {
        ...
        if (this.stateGame !== STATE_GAME.PLAYING) {
            require('GameManager').getInstance().user.lastGameID = 0;
            require('UIManager').instance.onShowLoad();
            cc.NGWlog("data levave game viwe la== " + JSON.stringify(this.dataLeave))
            if (typeof this.dataLeave != 'undefined') {
    ...
    getDynamicIndex(index) {
        ...
        } else if (require('GameManager').getInstance().curGameId === GAME_ID.BURMESE_POKER) {

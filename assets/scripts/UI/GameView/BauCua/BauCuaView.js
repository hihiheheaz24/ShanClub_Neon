...
var BauCuaView = cc.Class({
    ...
    handleStartGame(data) {
        ...
        if (this.stateGame != STATE_GAME.VIEWING) {
            require('TrackingManager').SendTracking(TRACKING_TYPE.PlayGame_);
            require('TrackingManager').SendTracking(TRACKING_TYPE.UserActive);
        }
        //Show animation
    ...
    handleFinish(packet) {
                ...
                for (let j = 0; j < this.listUserBet.length; j++) {
                    ...
                    if (gateIndex === x) {
                        ...
                        if (gateIndex < 7) {
                            let itemLast = [];
                            if (this.diceHistory.length > 0)
                                itemLast = this.diceHistory[this.diceHistory.length - 1];
                            ...
                            if (count === 2) {
                                ...
                                this.spTable.node.addChild(labelText.node, 12);
                                labelText.string = GameManager.getInstance().formatMoney(chip * count);
                            } else if (count === 3) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (N === GameManager.getInstance().user.uname) {
                ...
                let listMW = MW.split(';').map(Number);
                let totalMW = 0;
                for (let i = 0; i < listMW.length; i++) {
                    totalMW += listMW[i];
                }
                ...
                let aniWinlose = cc.callFunc(() => {
                    if (M > 0) {
                        ...
                        if (totalBetWin == 0) return;
                        require('SoundManager1').instance.playBauCuaWin();
                        this.spMask.node.active = true;
                        this.lbWin.string = 'WIN\n' + '+' + (GameManager.getInstance().formatMoney(totalMW * 2));
                        this.aniWin.node.active = true;
                        this.aniWin.setAnimation(0, "animation", false);
                    }
    ...
    flyChipBet(value, plName, gate, delayTime = 0) {
        ...
        //Create chip
        var chip = this.createChip(value, opos, npos, delayTime);
        return chip;
    },

    createChip(value, opos, npos) {
        //Create chip
        var count = 0;
    ...
    flyChip(chip, npos, delay = 0) {
        ...
        var rot = cc.rotateBy(this.TIMEACTION.CHIPFLY * 0.7, (Math.random() - 0.5) * 360);
    ...
    showChipBet() { // show chip bet cua minh
        ...
        for (let i = 0; i < ArrTG.length; i++) {
            ...
            this.list_lbBet[gate - 1].string = GameManager.getInstance().formatMoney(chip);
        }
    },
    ...
    showAllChipBet(plName, gate, ag) { //show chip bet cua tat ca
        ...
        if (plIndex === -1) {
            ...
            if (gate === 3 || gate === 5 || gate === 6) {
                ...
                nodeBet.position = this.btnBet[gate - 1].node.position.add(cc.v2(0, 80));
            } else if (gate === 13 || gate === 8 || gate === 15 || gate === 17 || gate === 14 || gate === 20 || gate === 10 || gate === 16) {
                nodeBet.position = this.btnBet[gate - 1].node.position.add(cc.v2(0, 30));
            ...
            labelText.cacheMode = 2;
            labelText.string = GameManager.getInstance().formatMoney(ag) + '/' + 1;
            ...
            userBet[5].getComponent(cc.Label).string = GameManager.getInstance().formatMoney(userBet[1]) + "/" + userBet[3].length;
        }
    },

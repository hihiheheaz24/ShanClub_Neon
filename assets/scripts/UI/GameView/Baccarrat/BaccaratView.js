...
var BaccaratView = cc.Class({
    ...
    handleDatCuoc(data) {
        ...
        let check = this.checkValueChip(data.chipBet);

        if (check) { // neu get dc chip thi tao 1 icon chip bay ra tu player.
    ...
    initIcChip(value) {
        let minBet = this.agTable;
        let valueChip = minBet;
        let listValue = [];
        listValue.push(valueChip, valueChip * 5, valueChip * 10, valueChip * 50, valueChip * 100);

        let spr = null;
        for (let k = this.listValue.length - 1; k >= 0; k--) {
            if (value == this.listValue[k]) {
        ...
        spr_ic_Chip.spriteFrame = spr;
        this.createLabelChip(icChip, value);

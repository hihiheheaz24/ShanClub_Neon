...
var packetCallback = function (packet) {
    ...
    switch (packet.classId) {
        ...
        case FB_PROTOCOL.GameTransportPacket.CLASSID:
            //if (require('GameManager').getInstance().curGameId !== GAME_ID.SHANKOEMEE) GameManager.getInstance().tableId = packet.tableid;
            ...
            //cc.NGWlog("tableid  tra ve ==================================: " + packet.tableid);
        ...
        case FB_PROTOCOL.ForcedLogoutPacket.CLASSID:
            {
                ...
                //      var byteArray = FIREBASE.ByteArray.fromBase64String(packet.message);
                // var message = utf8.fromByteArray(byteArray);
                // receiveDataFromService($.parseJSON(message));
                //  HandleServicePacket.handleService(message);
                break;
...
var loginCallback = function (status, playerId, screenname, credentials, code, message) {
    ...
    if (status == "OK") {
        ...
        cc.NGWlog("Chay vao login thanh cong");

        // {"evt":"0","data":"{\"Userid\":100425492,\"Username\":\"sondt123789\",\"Tinyurl\":\"sondt123789\",\"AG\":127280,\"LQ\":0,\"VIP\":1,\"MVip\":10,\"markLevel\":0,\"PD\":1500,\"OD\":1,\"A\":5,\"NM\":0,\"ListDP\":\"1500;2000;3000_3000;4000;5000_5000;6000;7000_7000;\",\"NewAccFBInDevice\":0,\"chipbank\":0,\"gameid\":0,\"NumFriendMail\":0,\"gameNo\":101,\"isShowAds\":false,\"UsernameLQ\":\"\"}","time":1539058407670,"auth":""}
        ...
        if (GameManager.getInstance().isChooseGame) {
            ...
            if (GAME_INFO[gameCur].isPlayNow || GameManager.getInstance().user.vip < 1 || require("GameManager").getInstance().isPlayNowBanner) {
                if (require("GameManager").getInstance().isPlayNowBanner) require("GameManager").getInstance().isPlayNowBanner = false;
        ...
        NetworkManager.getInstance().isCheckLoginAllSeverIP = false;
        // require("UIManager").instance.onShowConfirmDialog(GameManager.getInstance().getTextConfig('login_error'));
        require('UIManager').instance.onHideLoad();
        NetworkManager.getInstance().indexListSeverIp = 1;
        if (require("GameManager").typeLogin === LOGIN_TYPE.PLAYNOW) {
            if (NetworkManager.getInstance().isPlayNow) {
                require("TrackingManager").SendTracking(
                    TRACKING_TYPE.LoginPlayNowFail,
                    message
                );
            } else {
                if (code === -3) {
                    ...
                    // cc.sys.localStorage.removeItem("PASS_PLAYNOW");
                    // NetworkManager.getInstance().onPlayNow();
                }
                require("TrackingManager").SendTracking(
                    TRACKING_TYPE.ContinueFail,
                    message
                );
            }
        } else {
            if (require("GameManager").typeLogin === LOGIN_TYPE.FACEBOOK) {
                require("TrackingManager").SendTracking(
                    TRACKING_TYPE.FacebookFail,
                    message
                );
            } else {
                if (!require("TrackingManager").isTracking(TRACKING_TYPE.FacebookFail)) {
                    require("TrackingManager").SendTracking(
                        TRACKING_TYPE.RegisterFail,
                        message
                    );
                    if (code === -6) {
                        // V2Utils:: onFacebookLogin(1);
                        // this.isClickLogin = true;
                        require('Util').onLoginFb("1");
                        return;
                    }
                } else {
                    require("TrackingManager").SendTracking(
                        TRACKING_TYPE.LoginFail,
                        message
                    );
                }
            }
...
var statusCallback = function (status) {
    ...
    switch (status) {
        case FIREBASE.ConnectionStatus.CONNECTED: 
            {
                ...
                // if (isAutoLogin === 'false' && isAutoLogin != null) {/////
                //cc.NGWlog("on reconnect");
                cc.NGWlog("gia tri auto login la== " + isAutoLogin + " type:" + GameManager.getInstance().typeLogin);
                // cc.NGWlog('type login tu dong la============ ' + cc.sys.localStorage.getItem('lastLogin'));
                ...
                cc.NGWlog("Check Auto Login isInGame la: ", NetworkManager.getInstance().isInGame);
                cc.NGWlog("Check Auto Login isReconnect la: ", GameManager.getInstance().isReconnect);
                cc.NGWlog("Check Auto Login isAutoLogin la:", isAutoLogin);
                ...
                if (NetworkManager.getInstance().isInGame || GameManager.getInstance().isReconnect || isAutoLogin === "false") {
                    ...
                    switch (GameManager.getInstance().typeLogin) {
                        ...
                        case LOGIN_TYPE.NORMAL:
                            cc.NGWlog("Type auto login normal");
                            NetworkManager.getInstance().onLogin(
                        ...
                        case LOGIN_TYPE.PLAYNOW:
                            ...
                            break;
                        case LOGIN_TYPE.APPLE_ID:
                            if (require('GameManager').getInstance().identityTokenAppleID == '') {
                                Global.LoginView.onClickLoginAppleID();
                            } else {
                                NetworkManager.getInstance().onLogin("1", require('GameManager').getInstance().identityTokenAppleID, false, false);
                            }
                            break;
                    }
                }
        ...
        case FIREBASE.ConnectionStatus.DISCONNECTED:
            {
                ...
                GameManager.getInstance().isLoginOnGame = false;
                GameManager.getInstance().isDisconnect = true;
                if (require('GameManager').getInstance().gameView != null) {
                    console.log("Game view khac null");
                    console.log("State Game la: " + require('GameManager').getInstance().gameView.stateGame);
                    if (require('GameManager').getInstance().gameView.stateGame == STATE_GAME.PLAYING) {
                        console.log("Neu o trong game thi gan lại disconect = false");
                        GameManager.getInstance().isDisconnect = false;
                    }
                } 
        ...
        case FIREBASE.ConnectionStatus.CONNECTING:
            {
                cc.NGWlog("chay vao network connecting");
                NetworkManager.getInstance().statusConnect = status;
...
var NetworkManager = cc.Class({
    ...
    connect_sv(ip, port) {
        ...
        if (ip === null || ip === "") ip = "socket.app01.shweyang.com";
        // ip = "35.247.178.163"
        ...
        if (window.WebSocket && window.WebSocket.prototype && window.WebSocket.prototype.send) {
            ...
            // connector.connect("FIREBASE.WebSocketAdapter", "test.app01.shweyang.com", port, '', true); //svtest
        } else {
        ...
        if (cc.sys.isBrowser) {
            window.fbAsyncInit = function () {
                FB.init({
                    ...
                    appId: "1344223805760409", //shanClub
    ...
    onLogin(user, pass, isReg, isConnect = true) {
        ...
        if (languaSave !== null) {
            indexLanguae = parseInt(languaSave) == 0 ? "en" : "cam";
        }
        ...
        var user1 = {
            Userid: 1,
            From: "mbacay",
            //   gameid: 9501, // test bljack
            gameid: cc.sys.localStorage.getItem("curGameIdShanClub"), // 9501, //cc.sys.localStorage.getItem("curGameIdShanClub"),

            deviceId: GameManager.getInstance().deviceId,
            Signid: "qazwsxedcrfv123$%^789",
            loginCount: 0,
            versionCode: GameManager.getInstance().versionGame,
            language: indexLanguae,
            Username: user,
            Usertype: 1
        };
        if (isReg) {
        ...
        var jsonValue = JSON.stringify(user1);
        ...
        require('SMLSocketIO').getInstance().emitSIOWithValue(user1, "LoginPacket", true);
    },
    ...
    sendDataGame(data) {
        ...
        packet.pid = GameManager.getInstance().user.id;
    ...
    sendSelectGame(gameId) {
        ...
        for (let i = 0; i < GameManager.getInstance().listIp.length; i++) {
            if (GameManager.getInstance().listIp[i].gameID === gameId) {
                severIp = GameManager.getInstance().listIp[i].domain;
                cc.NGWlog('server ip=' + severIp);
                break;
            }
        }
        let curSV = cc.sys.localStorage.getItem("curServerIp" + NAME_GAME);
        cc.NGWlog('curSV ip=' + curSV);
        cc.NGWlog( 
            "----------------------------sendSelectGame--------------------"
        );

        if (severIp !== curSV) {
            cc.NGWlog('----------------- Da co 1 su khac biet lon!!!');
            GameManager.getInstance().isLogOut = false;
            this.isInGame = true;
            GameManager.getInstance().isChooseGame = true;
            this.onLogout();
        ...
        cc.sys.localStorage.setItem("curGameIdShanClub", gameId);
        cc.sys.localStorage.setItem("curServerIp" + NAME_GAME, severIp);
        GameManager.getInstance().curServerIp = severIp;
        GameManager.getInstance().curGameId = gameId;
    },

    sendSelectG2(gameId) {
        cc.NGWlog("================>click vao may lan select g2 ??? ");
        setTimeout(() => {
            GameManager.getInstance().is_send_ltv = false;
        }, 1000);
        var data = {
            evt: "selectG2",
            gameid: gameId
        };
        if(!GameManager.getInstance().is_send_ltv){
            cc.NGWlog('=======================> send selectg2 ig game' + gameId);
            this.sendService(JSON.stringify(data));
            GameManager.getInstance().is_send_ltv = true;
        }
    },
    ...
    sendDTConfirm(value, idW, type){
        var data = {
            evt: "cashOut",
            Id: GameManager.getInstance().user.id,
            CashValue: value,
            WaveID: idW,
            telco: type,
        };
    ...
    sendIAPFacebookInstant(_signedData, _signature) {
        ...
        // sendService(obj);
        cc.NGWlog("iap_fb:=====" + _signedData + _signature);
        var data = {
            evt: "iap_fb",

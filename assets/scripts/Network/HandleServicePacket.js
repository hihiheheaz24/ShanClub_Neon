            ...
            if (jsonData.hasOwnProperty("evt")) {
                ...
                switch (evt) {
                    ...
                    case SERVICE_CODE.ERR_JOIN_TABLE:
                        ...
                        if (Global.RegisterPopup.node.getParent() != null) Global.RegisterPopup.btn_confirm.interactable = true;
                        break;
                    ...
                    case "message":
                        ...
                        if (require('MailViewMessage').instance !== null) {
                            ...
                            if (require('GameManager').getInstance().gameView !== null)
                                require('GameManager').getInstance().onShowConfirmDialog(require('GameManager').getInstance().getTextConfig('has_mail'));
                            else {
                        ...
                        if (Global.MailView.node.getParent() != null) {
                            cc.NGWlog("chay vao reload mail")
                    ...
                    case "lottery_topgame":
                        if (!jsonData.data) return;
                        Global.LotoView.loadListTopLoto(jsonData.data);
                        break;
                    
                    case "lottery_lotos":
                        if (!jsonData.data) return;
                        Global.LotoView.loadListBet(jsonData.data);
                        break;

                    case "lottery_results":
                        if (!jsonData.data) return;
                        Global.LotoView.loadListResult1d(jsonData.data);
                        Global.LotoView.loadListResult2d(jsonData.data);
                        break;
                    ...
                    case "lottery_create":
                        ...
                        Global.LotoView.lb_ag.string = xyz.ag;
                        GameManager.getInstance().user.ag = xyz.ag;
                    break;
                    case "lottery_cancel":
                        if (!jsonData.data) return;
                        let abc = JSON.parse(jsonData.data);
                        Global.LotoView.lb_ag.string = abc.ag;
                    ...
                    case SERVICE_CODE.CHATWORLDMESS:
                        ...
                        if (Global.ChatWorldView.list_data_chat_world.length > 30) {
                            Global.ChatWorldView.list_data_chat_world.shift();
                        }
                        ...
                        item.level = jsonData.level;
                        if(item.type === 1){
                            Global.ChatWorldView.list_data_chat_world.push(item);
                            Global.MainView.addItemChatWorldMain(item);
                            cc.NGWlog('==================> chay vao update data chatwworld');
                        }
                        if(item.type === 2)
                        {
                            Global.LobbyView.list_data_chat_game.push(item);
                            cc.NGWlog('==================> chay vao update data chat game');
                        }
                        if(Global.ChatWorldView.node.getParent() !== null){
                            Global.ChatWorldView.updateChatItem(item);
                            cc.NGWlog('==================> chay vao update chatwview');
                        }
                        
                        if(Global.LobbyView.node.getParent() !== null){
                            Global.LobbyView.updateChatItem(item);
                            cc.NGWlog('==================> chay vao update chatLobbyView');
                        }
                    ...
                    case 'topgamer_new':
                        ...
                        if (typeof list !== 'undefined' && Global.TopGameView.node !== null && list.length > 0) {
                    ...
                    case 'rejectCashout': {
                        ...
                        if (jsonData.status == 0) Global.ExchangeView.onClickHistory();

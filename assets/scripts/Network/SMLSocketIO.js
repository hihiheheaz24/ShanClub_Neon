...
var SMLSocketIO = cc.Class({
  ...
  startSIO() {
    ...
    let url = cc.sys.localStorage.getItem('u_SIO');
    if (url == '' || url == null) url = '';
    //url = "http://192.168.1.114:3000/1st";//hien cmt;
    console.log("Url_SIo la: " + url);
  ...
  emitSIOCCC(strData) {
    ...
    //   return;
    // }
    //behaviorI: (behavior Ignore) evt nào có trong đây thì ko bắn lên  (bắn lên "behavior")
    //   if(std::find(blackListBehaviorIgnore.begin(), blackListBehaviorIgnore.end(), evtName) != blackListBehaviorIgnore.end() || std::find(blackListBehaviorIgnore.begin(), blackListBehaviorIgnore.end(), "all_sio") != blackListBehaviorIgnore.end()){
    //     cc.NGWlog("\n\n SIO: emitSIOCCC EVT NAY THUOC DIEN CHINH SACH KO DUOC GUI DI 🙁 -  evt: %s", evtName.c_str());
    //     return;
    // }
    // cc.NGWlog("strData la " + strData);
  ...
  emitLogin() {
    ...
    let ip = cc.sys.localStorage.getItem("curServerIp" + NAME_GAME) || "socket.app01.shweyang.com";
